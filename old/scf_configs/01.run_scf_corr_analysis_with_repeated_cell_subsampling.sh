...
for ka in ${kaList[@]}; do
	...
	# for k in ${knnList[@]}; do
		...
		# prep config file
		output="config_mc_scsmart_ka${ka}_knn${knn}_${date}.py"
		cp config_template_mc_scsmart_ka_knn.py $output 
		...
		for (( i=0; i<${subsample_times}; i++ )) do
			# # run corr analysis
			echo $i
			../correlation_analysis_celllevel-v3-stage3.py -ka $ka -knn $knn -dt $date -isub $i
			# break
		done
