...
if __name__ == '__main__':
    ...
    input_bundle_dirc = '/cndd2/fangming/projects/scf_enhancers/data/organized_cell_level/version_aug25'
    bundle_fnames = (
    ...
    logging.info("{}_{}_{}_{}".format(knn_xy.shape, knn_xx.shape, X.shape, Y.shape,))
    ...
    # enhancer by metacell (## need normalization? ##)
    ec_atac = Y.dot(knn_xy.T).todense() 

    logging.info("{}_{}".format(gc_rna.shape, ec_atac.shape))
    ...
    ec_atac_zscore = stats.zscore(np.array(ec_atac), axis=1, ddof=0,)


    # In[9]:


    # use ec_atac and gc_rna
    ...
    # enh_idx = snmcseq_utils.get_index_from_array(common_enhancer_regions.index.values, enhancer_gene_to_eval['ens']) # be careful here!
    enh_idx = snmcseq_utils.get_index_from_array(np.arange(len(ec_atac)), enhancer_gene_to_eval['ens']) # be careful here!
    ...
    # corr
    corrs = row_dot_product_norm_by_numcol(gc_rna_zscore, ec_atac_zscore, gene_idx, enh_idx)
